...
namespace MsdevManager {
    ...
    public class Msdev {
        ...
        public static void ShowExcel(IntPtr appHandle) {
            //System.IntPtr hWnd = (System.IntPtr)workbook.MainWindow.HWnd;
            if (IsIconic(appHandle)) {
                ShowWindowAsync(appHandle, SW_RESTORE);
            }
            SetForegroundWindow(appHandle);
        }

        public static void ShowExcel(Workbook workbook) {
        ...
        /// <param name="openSolutionsOnly">Only return instances that have opened a solution</param>
        /// <return
        public static Hashtable GetExcelInstances(bool openSolutionsOnly) {
            Hashtable runningIDEInstances = new Hashtable();
            Hashtable runningObjects = GetRunningObjectTable();
            ...
            while (rotEnumerator.MoveNext()) {
                string candidateName = (string)rotEnumerator.Key;
                if (!(candidateName.EndsWith(".xlsx") || candidateName.EndsWith(".xlsm") || candidateName.EndsWith(".xls")))
                    continue;
                Workbook excel = rotEnumerator.Value as Workbook;
                if (excel == null)
                    continue;

                if (openSolutionsOnly) {
                    try {
                        string solutionFile = excel.Name;   /*excel.Solution.FullName;*/
                        if (solutionFile != String.Empty) {
                            runningIDEInstances[candidateName] = excel;
                        }
                    }
                    catch { }
                }
        ...
        public static IntPtr getExcelWindowHandleByWindowName(string WindowName) {
            IntPtr objectHandle = default(IntPtr);
            WindowInfo[] wndList = GetAllDesktopWindows();
            foreach (WindowInfo wnd in wndList) {
                if (wnd.szWindowName == WindowName + " - Excel") {
                    ...
                    objectHandle = wnd.hWnd;
                    return objectHandle;
                }
            }
            return objectHandle;
        }
        public static IntPtr getExcelWindowHandleByClassName(string className) {
            IntPtr objectHandle = default(IntPtr);
            WindowInfo[] wndList = GetAllDesktopWindows();
            foreach (WindowInfo wnd in wndList) {
                if (wnd.szClassName == className) {
                    objectHandle = wnd.hWnd;
                    return objectHandle;
                }
                //Console.WriteLine("Window handle=" + wnd.hWnd.ToString().PadRight(20) + " szClassName=" + wnd.szClassName.PadRight(20) + " szWindowName=" + wnd.szWindowName);
